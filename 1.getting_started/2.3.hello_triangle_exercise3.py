...
class Window(window.Window):
    ...
    def setup(self):
        ...
        self.shader_orange = shaders.compileProgram(
            shaders.compileShader(vertex_shader,    gl.GL_VERTEX_SHADER),
            shaders.compileShader(fragment_shader_orange,  gl.GL_FRAGMENT_SHADER),
        )

        self.shader_yellow = shaders.compileProgram(
            shaders.compileShader(vertex_shader,    gl.GL_VERTEX_SHADER),
            shaders.compileShader(fragment_shader_yellow,  gl.GL_FRAGMENT_SHADER),
        )
    ...
    def on_draw(self, time, frame_time):
        ...
        gl.glUseProgram(self.shader_orange)
        gl.glBindVertexArray(self.vaos[0])
        gl.glDrawArrays(gl.GL_TRIANGLES, 0, 3)

        gl.glUseProgram(self.shader_yellow)
        gl.glBindVertexArray(self.vaos[1])
        gl.glDrawArrays(gl.GL_TRIANGLES, 0, 3)
